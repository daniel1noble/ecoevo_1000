...
- First, authors need not be afraid of sharing data and code early in the publication process. Sharing data and code early can help improve the quality of research, establish precedence, and improve the transparency and computational reproducibility of scientific findings. If authors are worried about data being used unintentially, clear information surrounding its re-use and withholding some data or meta-data needed to understand the dataset may be options. 
- Second, authors need to be aware of the benefits of posting preprints and postprints. Posting preprints is expected to help: 1) establish precedence of research findings; 2) ensure that research is open access and 3) authors receive feedback on their research before it is published. While it is clear points 1 and 2 have already been achived discussions around a preprints findings are still rare. It is clear that we, as a community, need to provide better platforms that document discussions around a preprint findings. Such discussions are useful for the authors to help improve their work and communicate their findings more effectively (when done constructively, of course).
- Third, authors need to be aware of the benefits of using community driven preprint servers. Community driven preprint servers are capable of setting their own agenda and are driven by the needs and disires of the community they would like their findings to be shared with. Supporting these initiatives should be a priority for all researchers. 
- Lastly, it remains clear that the time between preprinting and publication is still quite long. One possible explanation is that preprints are not being sent to journals deemed suitable for the work. Interestingly, out of the `r nrow(data2)` preprints, only `r pci[2,2]` (`r pci[2,3]*100`%) used Peer Community In (PCI) as a preprint peer-review service. Using PCI provides authors with quicker reviews of a paper and a more realistic recommendation of where such work is likely to be published. Many journals now also accept PCI reviews and recomendations when deciding to publish a paper. As such, we would encourage more authors to consider sending their preprints for PCI review to possibly speed up the publication process. 

While most preprints seemlessly get connected with their published version and merged together often they can remain disconnected making it unclear how to associate the preprint and published version, creating confusion in the findings and leading to frustration from authors. Cross-refererencing problems do occur and are the major reason for why preprints are not always connected automatically. Thanksfully, the reasons are fairly well understood and easily rectified. They often result from a mismatch between titles or authors in preprint and published metadata. In fact, it was reasonably common for titles of preprints to change between the preprint and the published article (`r titles[2,2]` or `r titles[2,3]*100`%) and this was the major contributer to preprints and published articles not being matched automatically in Google Scholar during our data collection. As such, we would recommend that authors make sure that, when preprints end up getting published, that they update their preprint with the publication DOI when it's available. Particularly if their title has changed. This is quite easy for authors to do in *EcoEvoRxiv* and it will ensure that the preprint is linked to the published article and the citations are merged and the published paper is ultimately open access, allowing authors to save open access fees. 

Despite the early successes of the new initiatives taken by *EcoEvoRxiv* it is still clear that much work needs to be done to improve our communities understanding of preprints (and postprints). We view this perspective piece as one small step in achieving that goal. We believe that the future of preprints is bright and community driven initiatives, such as *EcoEvoRxiv*, are set to play a key role in the future of scientific publishing.
