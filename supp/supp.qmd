...
# Summary of the data
sum_taxa <- data2  %>% select(all_of(cols))  %>% apply(., 2, sum, na.rm = TRUE)  %>% data.frame()   %>% rownames_to_column(var = "taxa")  %>% rename("n" = ".")
...
pie <- sum_taxa  %>% ggplot(aes(x = "", y = n, fill = taxa)) + geom_bar(stat = "identity") + coord_polar("y", start=0) + blank_theme + labs(title = "Taxa studied in preprints") + theme(legend.position = "bottom")
