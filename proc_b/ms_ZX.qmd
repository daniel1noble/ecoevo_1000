...
# Dataframes
     ...
     # Contains only preprints and not postprints (Does not contain preprints that haven't been published! -ZX)
              
     preprints_pub_only <- data2  %>% filter(time_between_preprint_and_pub_days >= 60 & !is.na(preprint_published_date)) 
     # Contains only preprints that have been published
     
     all_preprints <- data2  %>% filter(time_between_preprint_and_pub_days >= 60 | is.na(preprint_published_date))
     # Contains preprints that have and have not been published -ZX 
...
##---------------------##
  ...
  taxa_fig <- ggplot(sum_taxa, aes(x = fct_reorder(taxa, perc), y = perc)) +
    ...
    #coord_polar(theta = "y") + 
    theme_classic() + labs(y = "Percentage (%) of articles", x = "Studied taxa") + 
    theme(plot.tag = element_text(size = 16, face = "bold")) + coord_flip()
  

##---------------------##
# Types of articles (preprints AND postprints)
##---------------------##
# Summary of the data
...
academic_age <- ggplot(preprints, aes(x = academic_age)) +
  ...
  scale_y_continuous(expand = c(0, 0)) +
  labs(x = "Years since submitting author's first publication (academic age)", y = "Number of articles") +
...
#unpub <- preprints  %>% filter(is.na(publication_doi))  %>% nrow() #ZOE REMOVED BECAUSE "preprints" DOESN'T INCLUDE UNPUBLISHED PREPRINTS

unpub <- all_preprints %>% filter(is.na(publication_doi)) %>% nrow() #196 -ZX
total_preprints <- all_preprints %>% nrow() #945 -ZX

perc_unpub <- (unpub/total_preprints)*100 #percentage of unpublished preprints - ZX

#unpub number changes if you do filter(is.na()) by a different parameter (eg., publication_date) - ZX
