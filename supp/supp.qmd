...
## testing - delete thi later
#gender(c("Madison", "Hillary"), years = 2000, method = "ssa")
#gender(c("Madison", "Hillary"), method = "genderize") #works ok until limit of 100 is reached
#gender(data3$submitting_author_firstname, method = "genderize") #works ok until limit of 100 is reached

# data3 <-  data2[1:100, ] #subset for testing
# gender(data3$submitting_author_firstname) -> data3_genders
# dim(data3_genders) #83 names assigned
## EDN testing
...
# Plot probabilities of gender assignment
ggplot(data3, aes(x = `proportion_male`)) +
  geom_histogram(fill = "grey", color = "black") +
  labs(x = "gender assignment probability (male)", y = "Number of preprints") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

table(data3$proportion_male > 0.05 & data3$proportion_male < 0.95) #84 names bit unsure
