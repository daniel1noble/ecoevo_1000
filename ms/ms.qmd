...
In addition to speeding up dissemination, preprints can also be a useful way to ensure that research remains open and accessible to the research community because the paper is available online no matter what the open access status of the publication ends up being [@bourne2017ten; @vale2015accelerating]. Using the published articles on *EcoEvoRxiv*, we evaluated the open access status of each paper using the R package *roadoi* (vers. `r utils::packageVersion("roadoi")`) to connect to the Unpaywall platform [@jahn2024]. Most of the published articles arising from preprints were open access [`r sum_is_oa$n[2]` (`r sum_is_oa$percent[2]*100`%) [@fig-pubsummary]A]; however, the status of many were unknown [`r sum_is_oa$n[3]` (`r sum_is_oa$percent[3]*100`%)], and `r sum_is_oa$n[1]` (`r sum_is_oa$percent[1]*100`%) were published behind paywalls. The type of open access for a published paper in an open-access journal also varied widely ([@fig-pubsummary]B). 

Data and code sharing are also key components of open science [@roche2015public]. In the spirit of 'openness', we expected data and code sharing among preprints to be greater than in many papers published in research journals [@roche2015public; @o2021preferred]. Despite this, we found that `r preprint_data[1,2]` (`r preprint_data[1,"percent"]*100`%) of preprints relying on data on *EcoEvoRxiv* did not share data, and `r preprint_code[1,2]` (`r preprint_code[1,"percent"]*100`%) did not share code. Authors may be reluctant to share data and code for preprints because of the perceived concern that others may acquire and use such data and code prior to publication in a journal. Notably, authors of `r pre_article_data[3,2]` (`r pre_article_data[3,"percent"]*100`%) preprints that did not share data at the preprint stage ultimately did share data when the article was published; authors for `r pre_article_data[2,2]` (`r pre_article_data[2,"percent"]*100`%) preprints never shared data. The same was true for code. Overall, `r pre_article_code[3,2]` (`r pre_article_code[3,"percent"]*100`%) preprints had no open code at the preprint stage but did at the published article stage; authors of `r pre_article_code[2,2]` (`r pre_article_code[2,"percent"]*100`%) preprints did not share code on publication. Code and data-sharing patterns in our sample of preprints are consistent with analyses sharing practices for published articles (e.g., [@o2021preferred]), even for journals with strict public data archiving policies [@roche2015public].
